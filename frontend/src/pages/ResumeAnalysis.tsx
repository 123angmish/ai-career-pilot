...
const AnalysisSkeleton: React.FC = () => (
  <div className="space-y-6 shimmer">
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4 p-6 rounded-3xl bg-slate-100 border border-slate-200">
...
const ErrorState: React.FC<{ message: string; onRetry: () => void }> = ({ message, onRetry }) => (
  <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-rose-200 p-8 shadow-sm">
    <div className="h-14 w-14 rounded-2xl bg-rose-50 border border-rose-200 flex items-center justify-center mb-4">
      <AlertCircle className="h-7 w-7 text-rose-600" />
    </div>
    <h3 className="text-lg font-bold text-slate-900 mb-1 font-display">Analysis Failed</h3>
    <p className="text-sm text-slate-500 max-w-sm mb-6">{message}</p>
    <Button variant="outline" onClick={onRetry} className="border-slate-200 bg-white text-slate-700 hover:bg-slate-50">
...
const EmptyState: React.FC = () => (
  <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
    <div className="h-16 w-16 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-5 shadow-2xs">
      <FileCheck className="h-8 w-8 text-indigo-600" />
    </div>
    <h3 className="text-xl font-extrabold text-slate-900 mb-2 font-display">No Resume Analyzed Yet</h3>
    ...
    <Link to="/resumes/upload">
      <Button variant="primary" className="bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl shadow-sm">
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="relative">
        <pre className="text-xs text-slate-700 whitespace-pre-wrap font-mono leading-relaxed bg-slate-50 rounded-2xl p-4 overflow-hidden border border-slate-200">
          {expanded ? text : preview + (text.length > 400 ? '…' : '')}
        </pre>
        {text.length > 400 && (
          <button
            onClick={() => setExpanded(!expanded)}
            className="mt-2 flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700 transition-colors font-semibold"
          >
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          >
            {isFetching ? (
              ...
              <RefreshCw className="h-3.5 w-3.5 mr-1.5 text-indigo-600" />
            )}
            {isFetching ? 'Analyzing…' : 'Re-analyze'}
          </Button>

          <Link to="/resumes/jd-match">
            <Button size="sm" id="btn-jd-match" className="bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl shadow-sm">
              JD Matcher
      ...
      <AnimatePresence mode="wait">
        {isLoading && (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="flex items-center gap-3 p-4 mb-4 rounded-2xl bg-indigo-50 border border-indigo-100 text-indigo-800 text-sm font-medium">
